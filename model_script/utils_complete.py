...
def extract_feat(X,
    ...
    expo = np.sqrt( np.sum( (X - mu)**2, axis= 0 ) )
    h = np.exp( -1*expo/var )
    return h
...
def trn_algrm(X,
    ...
    while (flag == 0 and count < max_itr):
        count += 1
        [y, logits] = model(params, X) # Pass data and get predictions
        ...
        if loss < thrshold: # Check if target loss is reached
            flag  = 1
        d = deri(X, y, T) # Compute derivative
        # Update weights
        params = params - (lr*d) # Gradient descent update rule
